...
namespace eZet.EveLib.Test {
    ...
    public class Character_StaticDeserializationTests {
        ...
        public Character_StaticDeserializationTests() {
            ...
            _character.RequestHandler = new RequestHandler(new StaticXmlRequester(), new XmlSerializerWrapper());
        }

        [TestMethod]
        public void GetCharacterInfo() {
            EveApiResponse<CharacterInfo> xml = _character.GetCharacterInfo();
            Assert.AreEqual(99999999, xml.Result.CharacterId);
        }

        [TestMethod]
        public void GetAccountBalance() {
            EveApiResponse<AccountBalance> xml = _character.GetAccountBalance();
            Assert.AreEqual(4759, xml.Result.Accounts.First().AccountId);
        }

        [TestMethod]
        public void GetAssetList() {
            EveApiResponse<AssetList> xml = _character.GetAssetList();
